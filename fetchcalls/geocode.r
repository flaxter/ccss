...
geocoded = data.frame(data,geocoded.tracts$TRACTCE10,geocoded.tracts$COMMAREA)
#names(geocoded) = c(names(data),"block","beat","tract","area")
#names(geocoded) = c(names(data),"beat","tract","area")
names(geocoded) = c(names(data),"tract","area")
write.csv(geocoded, "311-geocoded.csv",row.names=FALSE, quote=FALSE)
